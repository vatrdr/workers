    ...
    protected function process(array $data, string $ts): PromiseInterface {
        ...
        return coroutine(function() use ($data) {
            ...
            foreach($data as $pilot) {
                 ...
                 *  - not sure if it needs to happen at initial data ingest or elsewhere
                 *  - ingest w/ _orig vars holding unedited received data, maybe?
                 */
                ...
                $trackKey = sprintf('vtrack:%d:%s:%s:%s', $pilot->cid, $callsign, $departure, $arrival);
