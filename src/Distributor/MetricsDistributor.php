...
class MetricsDistributor implements WorkerInterface
{
    ...
    public function pilots(array $data): array
    {
        ...
        foreach ($data as $pilot) {
            ...
            if ($pilot->flightPlan) {
                ...
                if (array_key_exists($short, $aircraft)) {
                    $aircraft[$short]++;
                } else {
                    $aircraft[$short] = 1;
                }
    ...
    public function airports(array $data): array
    {
        ...
        foreach ($data as $pilot) {
            ...
            if (array_key_exists($depAirport, $departures)) {
                $departures[$depAirport]++;
            } else {
                $departures[$depAirport] = 1;
            }
            if (array_key_exists($arrAirport, $arrivals)) {
                $arrivals[$arrAirport]++;
            } else {
                $arrivals[$arrAirport] = 1;
            }
        }
